    ...
    do
        for n in 40 100 200
            do
            	...
            	for i in {1..9} 99
		    do
			 cat ../${eop}/CF6_16D/EVAL10000/N${n}_G${g}_T$((12*$n/100))/outputs/s${i}_nds.out >> cf616d_all_seeds_nds.out
		    done
            done
    done
    
lines=`wc -l < cf616d_all_seeds_nds.out`
    
echo -e "1\n0\n2\ncf616d_all_seeds_nds.out\n${lines}\n1\n1" > getReferencePoint10000EV.in
    ...
    do
        ...
        for n in 40 100 200
            do
            	...
            	for i in {1..9} 99
		    do
			 cat ../${eop}/CF6_16D/EVAL10000/N${n}_G${g}_T$((12*$n/100))/outputs/s${i}_nds.out > processData.out
			 echo -e "1\n0\n2\nprocessData.out\n${n}\n1\n0\n$hvrefx\n$hvrefy" > processData.in
			 ./metrics  < processData.in
			 hvi=`cut -f2 hypervol.out`
			 spci=`cut -f2 spacing.out`
			 echo -e "$hvi\t$spci" >> tmp_cf616d_${eop}/N${n}_G${g}/metric_NDS.out
		    done
            done
    done
...
python ../NDS_METRICSPROCESS_HV_SPC_10000EV_CF6_16D.py
