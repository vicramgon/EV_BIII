	...
	for eop2 in CMOEAD_ADAPT CMOEAD_THRES
		...
		for n in 40 80 100
		    do
		    	...
		    	for i in {1..9} 99
			    do
				 cat ../${eop1}/CF6_4D/EVAL4000/N${n}_G${g}_T$((10*$n/100))/outputs/s${i}_gen$(($g-1)).out > processData1.out
				 lines1=`wc -l < processData1.out`
				 cat ../${eop2}/CF6_4D/EVAL4000/N${n}_G${g}_T$((10*$n/100))/outputs/s${i}_gen$(($g-1)).out > processData2.out
	...
	for n in 40 80 100
		...
		for i in {1..9} 99
		    do
		        cat ../${eop1}/CF6_4D/EVAL4000/N${n}_G${g}_T$((10*$n/100))/outputs/s${i}_gen$(($g-1)).out > processData1.out
			lines1=`wc -l < processData1.out`
			tail -${n} ../NSGAII/CF6_4D/EVAL4000/P${n}G${g}/cf6_4d_all_popmp${n}g${g}_seed0${i}.out > processData2.out
			lines2=`wc -l < processData2.out`
			echo -e "2\n0\n2\nprocessData1.out\n$lines1\n1\nprocessData2.out\n$lines2\n1\n1" > processData.in
			./metrics  < processData.in
		  	csi=`cut -f2 cs.out`
...
python ../FGENMETRICSPROCESS_CS_4000EV_CF6_4D.py
