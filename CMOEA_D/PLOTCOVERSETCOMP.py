...
for i,s in enumerate([1,2,3,4,5,6,7,8,9,99]):
    plt.plot(np.arange(1, G1+1), 100*data1[G1*i:G1*(i+1),1], c=colors(0))
    plt.plot(np.arange(1, G2+1), 100*data2[G2*i:G2*(i+1),1], c=colors(1))
    
patch1 = mpatches.Patch(color=colors(0), label=f'C({ALG2},{ALG1})')
patch2 = mpatches.Patch(color=colors(1), label=f'C({ALG1},{ALG2})')
plt.title(f'Coverset Comp. {problem} {ALG1} N{N1} G{G1} vs {ALG2} N{N2} G{G2}\n', fontsize=10)
plt.xlabel('Generation', fontsize=11)
plt.ylabel('% of dominated', fontsize=11)
...
plt.savefig(f'../METRICS_PLOTS/{problem}/CoverSet_COMP_{problem}_{ALG1}N{N1}G{G1}_{ALG2}N{N2}G{G2}.png', dpi=200)
plt.clf()