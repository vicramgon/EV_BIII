...
for N,G in [(100, 100), (40,250),(200,50),(40,100),(80,50),(100,40)]:
    ...
    # FILES 
    outputDirPath, plotsDirPath = prepareDir(f'CMOEAD_THRES/CF6_4D/EVAL{N*G}/N{N}_G{G}_T{T}')
    ...
    for seed in list(range(1,10))+[99]:
        ...
        fig.suptitle(f"Development (Conv) CF64D_PENAL_N{N}_G{G}_T{T}_s{seed}\n")
        ...
        # Plot title, axis labels and grid
        fig.suptitle(f"Development (Feas) CF64D_PENAL_N{N}_G{G}_T{T}_s{seed}\n")
        plt.subplots_adjust(left=0.125,
                        bottom=0.125, 
                        right=0.9, 
                        top=0.9, 
        ...
        fig.suptitle(f"NDS CF64D_CMOEAD_PENAL_N{N}_G{G}_T{T}_s{seed}\n")
        ax1.set_xlabel('$f_1$', fontsize=11)
        ax1.set_ylabel('$f_2$', fontsize=11)
        ax1.grid(True)
        plt.subplots_adjust(left=0.125,
    ...
    #  Plot title, axis labels and grid
        ...
        ax1.legend(handles=legend_elements)
        fig.suptitle(f" CMOVEAD_PENAL VS SGAII (FGEN) CF64D_N{N}_G{G}_s{seed}\n")
        ax1.set_xlabel('$f_1$', fontsize=11)
        ax1.set_ylabel('$f_2$', fontsize=11)
        ax1.grid(True)
        plt.subplots_adjust(left=0.125,
...
for seed in list(range(9)) + [98]:
    ...
    for i, (N,G) in enumerate(PROOFS):
        T = T=(10*N)//100
        data = np.genfromtxt(f"CMOEAD_THRES/CF6_4D/EVAL{N*G}/N{N}_G{G}_T{T}/outputs/s{seed+1}_gen{G-1}.out", delimiter='\t')
    ...
    plt.subplots_adjust(left=0.125,
                    ...
                    wspace=0.35, 
                    hspace=0.35)
    if not(os.path.isdir(f"CMOEAD_THRES/CF6_4D/EVAL10000/COMP_PLOTS")):
        os.mkdir(f"CMOEAD_THRES/CF6_4D/EVAL10000/COMP_PLOTS")
    fig.savefig(f"CMOEAD_THRES/CF6_4D/EVAL10000/COMP_PLOTS/s{seed+1}_FGEN.png", dpi=600)
    plt.clf(); plt.close()
    ...
    for i, (N,G) in enumerate(PROOFS):
        T = T=(10*N)//100
        data = np.genfromtxt(f"CMOEAD_THRES/CF6_4D/EVAL{N*G}/N{N}_G{G}_T{T}/outputs/s{seed+1}_nds.out", delimiter='\t')
    ...
    if not(os.path.isdir(f"CMOEAD_THRES/CF6_4D/EVAL10000/COMP_PLOTS")):
        os.mkdir(f"CMOEAD_THRES/CF6_4D/EVAL10000/COMP_PLOTS")
    fig.savefig(f"CMOEAD_THRES/CF6_4D/EVAL10000/COMP_PLOTS/s{seed+1}_NDS.png", dpi=600)
    plt.clf(); plt.close()