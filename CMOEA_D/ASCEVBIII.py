      ...
      #  VP_ = deepcopy(VP)
        for cur in order:
            ...
            for j in sorted(B[cur], key=(lambda x: np.random.random())):
                gtey = np.max([L[j][i]*abs(fy[i]-z[i]) for i in range(m)]) 
                gtex = np.max([L[j][i]*abs(FP[j][i]-z[i]) for i in range(m)])
                
                
                if  not(vy) and (bool(VP[j]) or gtey<gtex):
                    P[j] = y
                    FP[j] = fy
                    VP[j] = vy
                    updations += 1
