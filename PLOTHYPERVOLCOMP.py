...
plt.title(f'Hyervolume Comp. {ALG1} N{N1} G{G1} vs {ALG2} N{N2} G{G2}\n')
plt.xlabel('Generation', fontsize=11)
plt.ylabel('Hypervolume', fontsize=11)
plt.legend(handles=[patch1, patch2])
plt.grid(True)

if not(os.path.isdir(f'../METRICS_PLOTS')):
    os.mkdir(f'../METRICS_PLOTS')

plt.savefig(f'../METRICS_PLOTS/Hypervol_COMP_{ALG1}N{N1}G{G1}_{ALG2}N{N2}G{G2}.png', dpi=200)
plt.clf()