...
# for eop in ['EOP1','EOP2','EOP3', 'NSGAII']:
#     for n,g in [(40,100),(80,50),(100,40)]:
#             data = np.genfromtxt(f"./tmp_zdt3_{eop}/N{n}_G{g}/metric_FGEN.out")
#             hvmean, spcmean = np.mean(data, axis=0)
#             hvstd, spcstd = np.std(data, axis=0)
#             resData.append((eop if eop == 'NSGAII' else f'MOEA/D {eop}' ,n,g,hvmean,hvstd, spcmean, spcstd))
...
resData=[]
algs = ['EOP1','EOP2','EOP3', 'NSGAII']
for i, eop1 in enumerate(algs):
    for eop2 in algs[i+1:]:
        for n,g in [(40,100),(80,50),(100,40)]:
                data = np.genfromtxt(f"./tmp_zdt3_{eop1}/N{n}_G{g}/metric_CS_{eop2}_FGEN.out")
